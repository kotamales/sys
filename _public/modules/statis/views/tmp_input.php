...
<div class="x_panel">
	...
	<div class="x_content">
		...
		echo lang('msg_'.$dat_edit['sts'].'_data'); echo $hide_field . $primary . $sts_simpan; 
		?>
	</div>
...
function draw($master, $data, $tab='', $next_row=false, $cols=''){
	...
	foreach($master['fields'] as $key=>$row)
	{
		...
		if ($sts_cols){
			$sts_in_array=(in_array($row['field'],$cols['field']))?true:false;
			$width="30%";
			$width_multi="310px";
		}
				...
				if($row['show'])
				{
					...
					if (array_key_exists('bind',$master)){
						foreach($master['bind'] as $bd){
							if ($bd['nmtbl']==$row['nmtbl'] && $bd['field']==$row['field']){
								...
								if (array_key_exists('btn_add',$bd)){$btn_add='<span class="btn btn-flat btn-small btn-default btn-add" data-target="'.$bd['target'].'" data-combo="'.$label.'" data-label="'.$msg_title.'" data-fld="'.$bd['fld'].'">'.lang('msg_tombol_add_list').' '.$msg_title.'</span>';}
								break;
							}
						}
					}
					...
					switch ($type){
						...
						case 'integer':
							...
							break;
						case 'float':
							$isi = number_format(floatval($isi),1);
							if (empty($isi))
								$isi=0;
						...
						case 'upload':
							...
							if (!empty($isi)){
								...
								if ($ext=="jpg" || $ext=="png" || $ext=="jpeg" || $ext=="gif" || $ext=="bmp"){
									$o = '<img id="img_'.$label.'" style="width:40%; margin-top:10px;"  src="'.$nm($isi).'" alt="image"/>';
								}
				...
				<?php 
					...
					?>
						<div class="form-group clearfix <?php echo $error;?>">
							<label class="col-sm-3 control-label text-left" id="<?=$label.'_label';?>" style="text-align:left !important;">
