...
class Data extends MX_Model {
	...
	function get_data_posisi_head(){
		
		$this->db->select('*');
		$this->db->from(_TBL_HEAD);
		$this->db->order_by('urut');
		$query=$this->db->get();
		$rows=$query->result_array();
		$input=array();
		foreach($rows as $row){
			$input[] = array("id" => $row['id'], "title" => $row['judul'], "slug" => $row['pid'], "pid" => $row['pid'], "urut" => $row['urut'], "aktif" => $row['aktif']);
		}
	...
	function simpan_data($data){
		...
		foreach($rows as $row) { 
			...
			$where['id']=$row->id;
			// Doi::dump($upd);
			$result=$this->crud->crud_data(array('table'=>_TBL_DEPARTEMEN, 'field'=>$upd,'where'=>$where,'type'=>$type));
